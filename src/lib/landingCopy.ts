// landingCopy.ts
export const landingCopy = {
  seo: {
    title:
      "Eficiência Operacional, Automação Inteligente e Governança | Jadson Campos",
    description:
      "Arquiteto de Soluções Operacionais. Diagnóstico orientado a dados para identificar restrições estruturais, eliminar fricções operacionais e escalar com governança. Restaurantes, clínicas, hospitais e serviços recorrentes.",
    keywords: [
      "eficiência operacional",
      "automação inteligente",
      "automação de processos",
      "melhoria de processos",
      "restrições estruturais",
      "fricções operacionais",
      "governança operacional",
      "process mining",
      "mineração de processos",
      "autoatendimento",
      "arquitetura de serviços",
      "KPIs e SLAs",
      "redução de custos operacionais",
    ],
  },

  cta: {
    label: "Agendar diagnóstico no WhatsApp",
    helper: "Conversa inicial objetiva. Sem compromisso. Sem proposta automática.",
    whatsappNumber: "5521988551085",
    whatsappMessage: "Olá, gostaria de agendar um diagnóstico operacional",
  },

  hero: {
    badge: "Arquitetura Operacional • Automação • Dados",
    h1: "Eficiência Operacional para escalar com controle e previsibilidade",
    subheadline:
      "Identifico restrições estruturais com base em dados e desenho soluções — processos, automação inteligente e arquitetura de serviços — para eliminar fricções, reduzir custo e aumentar previsibilidade.",
    forWho: "Para restaurantes, clínicas, hospitais e operações recorrentes.",
    signature:
      "Jadson Campos • Arquiteto de Soluções Operacionais • +20 anos em operações complexas e reguladas",
    highlights: [
      {
        icon: "chart",
        title: "Diagnóstico orientado a dados",
        description: "Identificação de restrições estruturais e perdas com análise objetiva",
      },
      {
        icon: "cog",
        title: "Automação inteligente",
        description: "BPM/RPA com governança e trilha de decisão",
      },
      {
        icon: "check",
        title: "Governança operacional",
        description: "KPIs acionáveis e rotinas de melhoria contínua",
      },
    ],
  },

  problems: {
    title: "Restrições estruturais recorrentes",
    guide: "Essas restrições aparecem em setores diferentes, mas produzem sempre os mesmos efeitos.",
    items: [
      {
        category: "Restrição âncora",
        text: "Sem governança, custo e experiência viram ruído — e decisão vira opinião.",
      },
      {
        category: "Custo",
        text: "Custo e desperdício invisíveis (ninguém enxerga causa)",
      },
      {
        category: "Dependência",
        text: "Dependência de pessoas-chave e exceções",
      },
      {
        category: "Rastreabilidade",
        text: "Baixa rastreabilidade entre áreas/canais (retrabalho)",
      },
      {
        category: "Indicadores",
        text: "Indicadores que medem atividade, não decisão",
      },
    ],
  },

  services: {
    title: "O que eu implemento",
    intro:
      "Não é consultoria genérica. É desenho e implantação de soluções operacionais com governança contínua.",
    items: [
      {
        title: "Arquitetura operacional orientada a dados",
        desc: "Diagnóstico estrutural, desenho de fluxos e processos ponta a ponta, definição de métricas acionáveis.",
      },
      {
        title: "Implantação de centrais e canais (digital e presencial)",
        desc: "Estruturação completa de operações de atendimento, SAC, suporte e backoffice com processos, equipes e SLAs.",
      },
      {
        title: "Implantação e customização de sistemas e CRMs",
        desc: "Salesforce, Zendesk, NICE e outras plataformas adaptadas ao contexto operacional real, não configuração genérica.",
      },
      {
        title: "Agentes, automações e fluxos inteligentes",
        desc: "Automações que leem métricas, sugestões acionáveis (sinais operacionais/mercado).",
      },
      {
        title: "Estruturação e reestruturação de CS & CX",
        desc: "Desenho e implanto sistemas de CX e CS escaláveis para reduzir churn, proteger receita e qualificar a experiência do cliente.",
        highlighted: true,
      },
      {
        title: "Governança contínua: custo, SLA, churn, reclamações, escala",
        desc: "Ciclo mensal de monitoramento, alerta, ajuste e decisão para manter previsibilidade operacional e financeira.",
        highlighted: true,
      },
    ],
  },

  howItWorks: {
    title: "Como funciona",
    steps: [
      {
        title: "1) Diagnóstico",
        desc: "Entendo o contexto, leio dados disponíveis e mapeio restrições estruturais, riscos e oportunidades.",
      },
      {
        title: "2) Desenho da solução",
        desc: "Defino fluxo, regras, métricas e arquitetura (processo + sistema + operação).",
      },
      {
        title: "3) Implantação e sustentação",
        desc: "Execução com governança: acompanhamento, ajustes e evolução contínua do padrão.",
      },
    ],
    note:
      "Foco em resultado mensurável: custo, previsibilidade e consistência de experiência.",
  },

  demo: {
    title: "Exemplo de leitura operacional orientada a dados",
    text:
      "Um recorte simplificado de como uso dados para identificar restrições estruturais e orientar decisões. A ferramenta não é o produto — é parte do método.",
    hint: "Inserção de dados → leitura → orientação de decisão",
    fieldsSuggestionTitle: "O que a demo pode pedir (exemplo)",
    fieldsSuggestion: [
      "Volume (atendimentos / pedidos / agendas)",
      "Tempo de ciclo (início → fim)",
      "Taxa de retrabalho / reabertura",
      "Capacidade (equipe / turnos / salas)",
      "Principais categorias de custo / consumo (quando aplicável)",
    ],
  },

  benefits: {
    title: "Resultados esperados (sem promessas irreais)",
    items: [
      "Redução de desperdício e custo operacional por eliminação de fricções estruturais e retrabalho",
      "Mais previsibilidade financeira e operacional (menos variação, mais controle)",
      "Decisão mais rápida com indicadores acionáveis",
      "Padronização para escalar com consistência",
      "Melhora perceptível na experiência do cliente como consequência da operação funcionando",
    ],
    disclaimer:
      "Resultados variam conforme contexto, maturidade e qualidade dos dados disponíveis.",
  },

  segments: {
    title: "Onde aplico com mais impacto",
    items: [
      {
        title: "Operações com CMV e margem sensível",
        desc: "Indústria, food, redes, operações metrificadas — identificação de perdas estruturais e controle de custo.",
      },
      {
        title: "Clínicas e hospitais (fluxo, agenda, redução de no-show)",
        desc: "Produtividade de agenda, redução de no-show, governança entre áreas, custo por procedimento.",
      },
      {
        title: "Serviços recorrentes e operações escaláveis",
        desc: "Processos replicáveis, automação pragmática, margens previsíveis, governança contínua.",
      },
      {
        title: "Operações intensivas em atendimento e reputação",
        desc: "Padronização de jornadas, redução de fricções, indicadores acionáveis, controle de SLAs.",
      },
    ],
  },

  authority: {
    title: "Autoridade (fatos)",
    items: [
      "+20 anos estruturando e transformando operações em ambientes complexos, regulados e de alta pressão",
      "Experiência transversal em setores como saúde, telecom, transporte e serviços essenciais",
      "Atuação conectando operação, dados, processos, tecnologia e governança para decisão executiva",
    ],
  },

  evidence: {
    // Deixe vazio até você ter fatos para preencher.
    // É melhor vazio do que inventado.
    title: "Evidências (preencher com fatos reais)",
    placeholders: [
      "Ex.: Case: redução de retrabalho em X% após padronização e automação (com contexto e período)",
      "Ex.: Antes/depois de indicador operacional (tempo de ciclo, reabertura, SLA, capacidade)",
      "Ex.: Escopo: unidades, volume, complexidade — sem citar cliente se houver NDA",
    ],
  },

  closing: {
    anchor:
      "Não digitalizo burocracia. Projeto sistemas de serviço que eliminam restrições estruturais, funcionam, escalam e geram retorno mensurável.",
    finalTitle: "Agende um diagnóstico operacional",
    finalText:
      "Conversa inicial orientada a dados para mapear restrições estruturais e definir um plano pragmático de solução. Sem compromisso. Sem proposta automática.",
  },
};
